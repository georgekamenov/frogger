...
const froggerImage = new Image(grid, grid);
froggerImage.src = "img/frogger.png";

const carImage = new Image(grid, grid);
carImage.src = "img/car.png";

const fastcarImage = new Image(grid, grid);
fastcarImage.src = "img/fastcar.png";

const truckImage = new Image(grid, grid);
truckImage.src = "img/truck.png";

const bulldozerImage = new Image(grid, grid);
bulldozerImage.src = "img/bulldozer.png";

const turtleImage = new Image(grid, grid);
turtleImage.src = "img/turtle.png";

const shortlogImage = new Image(grid, grid);
shortlogImage.src = "img/shortlog.png";

const logImage = new Image(grid, grid);
logImage.src = "img/log.png";

const longlogImage = new Image(grid, grid);
longlogImage.src = "img/longlog.png";

PlaySound = function () {
  var audio = new Audio('audio/frogger.mp3');
  audio.loop = true;
  audio.play(); 
}
...
Sprite.prototype.render = function() {
  ...
  if (this.shape === 'frogger') {
    context.drawImage(froggerImage, this.x, this.y, this.size, 48);
    return;
  }
  if (this.shape === 'car') {
    context.drawImage(carImage, this.x, this.y, this.size, 48);
    return;
  }
  if (this.shape === 'fastcar') {
    context.drawImage(fastcarImage, this.x, this.y, this.size, 48);
    return;
  }
  if (this.shape === 'truck') {
    context.drawImage(truckImage, this.x, this.y, this.size, 48);
    return;
  }
  if (this.shape === 'bulldozer') {
    context.drawImage(bulldozerImage, this.x, this.y, this.size, 48);
    return;
  }
  if (this.shape === 'turtle') {
    context.drawImage(turtleImage, this.x, this.y, this.size, 48);
    return;
  }
  if (this.shape === 'shortlog') {
    context.drawImage(shortlogImage, this.x, this.y, this.size, 48);
    return;
  }
  if (this.shape === 'log') {
    context.drawImage(logImage, this.x, this.y, this.size, 48);
    return;
  }
  if (this.shape === 'longlog') {
    context.drawImage(longlogImage, this.x, this.y, this.size, 48);
    return;
  }
}
...
const patterns = [
  ...
  // log
  {
    ...
    shape: 'log',     // shape of the obstacle (rect or circle)
